...
*/
//  I2C write routine
void I2Cwrite(int id,byte reg,byte *data,int datasize)
{
...
//  I2C read routine
void I2Cread(int id,byte reg,byte *data,int datasize)
{
  Wire.beginTransmission(id);
  Wire.write(reg);
...
void AddLEDdata(unsigned short pat, unsigned char a)
{
  for (int i = 0; i < 16; i++)
  {
    if ((pat & (0x0001 << i)) == (0x0001 << i)){
      led_dat[i] = (led_dat[i] + a) & 0x00FF;
    }
  }
}
